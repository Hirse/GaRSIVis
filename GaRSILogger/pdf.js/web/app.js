...
let PDFViewerApplication = {
  ...
  zoomIn(ticks) {
    ...
    this.eventBus.dispatch('renderer-zoom', this.pdfViewer.currentScale,
                           newScale);
  ...
  load(pdfDocument) {
    ...
    pdfDocument.getMetadata().then(({ info, metadata, }) => {
      ...
      const appTitle = document.title.replace(/.* - /, '');
      if (pdfTitle) {
        this.setTitle(pdfTitle + ' - ' + appTitle);
      } else {
...
// Remove usage of function as we open files from the node API
let webViewerOpenFileViaURL; // eslint-disable-line
if (typeof PDFJSDev === 'undefined' || PDFJSDev.test('GENERIC')) {
...
function webViewerScaleChanged(evt) {
  PDFViewerApplication.eventBus.dispatch('renderer-zoom',
    PDFViewerApplication.pdfViewer.currentScale, evt.value);
      ...
      case 96: // '0' on Numpad of Swedish keyboard
        if (!isViewerInPresentationMode) {
          ...
          setTimeout(function () {
            ...
            pdfViewer.eventBus.dispatch('renderer-zoom', pdfViewer.currentScale,
                                        DEFAULT_SCALE_VALUE);
          });
          handled = false;
        }
